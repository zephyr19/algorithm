...
public class RandomizedQueueNaive<Item> implements Iterable<Item> {
    private static final int INITIAL = 16;
    private Item[] value;
    private int size;

    // construct an empty randomized queue
    public RandomizedQueueNaive() {
        value = (Item[]) new Object[INITIAL];
        size = 0;
    }

    private RandomizedQueueNaive(RandomizedQueueNaive<Item> source) {
        size = source.size;
    ...
    public Iterator<Item> iterator() {
        return new RandomizedQueueNaiveIterator(this);
    }
    ...
    public static void main(String[] args) {
        RandomizedQueueNaive<Integer> randomizedQueue = new RandomizedQueueNaive<>();
        for (int i = 0; i < 10; i++) {
    ...
    private class RandomizedQueueNaiveIterator implements Iterator<Item> {
        RandomizedQueueNaive<Item> randomizedQueue;

        private RandomizedQueueNaiveIterator(RandomizedQueueNaive<Item> source) {
            randomizedQueue = new RandomizedQueueNaive<>(source);
        }
